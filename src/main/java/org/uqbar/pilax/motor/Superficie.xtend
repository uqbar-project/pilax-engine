...
class Superficie extends ImagenMotor implements Canvas {
	...
	def texto(String cadena, int x, int y, int magnitud) {
		texto(cadena, x, y, magnitud, null, Color.BLACK)
	}
	...
	def obtener_recuadro(double dx, double dy, double ancho, double alto) {
        val qi = imagen.toImage()
        val rect = new QRect(dx.intValue, dy.intValue, ancho.intValue, alto.intValue)
        val qii = qi.copy(rect)
        return new ImagenMotor(QPixmap.fromImage(qii))
    }
    
    def dibujarPunto(double x, double y) {
    	dibujarPunto(x, y, Color.black)
    }
    
    def dibujarPunto(double x, double y, Color color) {
        circulo(x.intValue, y.intValue, 3, color, true, 1)
    }
